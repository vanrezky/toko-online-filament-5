...
class PayrollMonthlyBillingAggregationTest extends TestCase
{
    ...
    public function test_it_aggregates_installment_and_full_bills_per_customer_for_target_month(): void
    {
        ...
        $this->assertEquals(300000.0, $summary['total_deduction']);
        $this->assertSame(1, $summary['total_installments']);
        $this->assertSame(1, $summary['total_full_bills']);

        $detail = collect($summary['details'])->first();
        $this->assertNotNull($detail);
        $this->assertEquals(300000.0, $detail['total_deduction']);
        $this->assertSame(1, $detail['active_installments']);
        $this->assertStringContainsString($installmentTransaction->code, $detail['references']);
        $this->assertStringContainsString($fullTransaction->code, $detail['references']);
    }

    public function test_it_excludes_full_bills_with_non_collectible_status(): void
    {
        $service = app(PayrollExportService::class);
        ...
        $this->assertEquals(0, $summary['total_deduction']);
