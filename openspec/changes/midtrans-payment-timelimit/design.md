...
The job locks and reloads the transaction, re-evaluates eligibility and UTC deadline, queries a supported external provider before cancelling, and treats a provider success/settlement as paid reconciliation rather than cancellation. A Midtrans HTTP 404 is a confirmed absence of the local UUID in the configured merchant environment, so it is classified as `not_found` and follows the unpaid cancellation path. Other provider lookup failures remain `unknown`; the job releases/retries rather than cancelling on uncertainty. The job then invokes the existing `TransactionCancellationService` exactly once and dispatches expiry notification only after successful cancellation. Duplicate scans/jobs become no-ops.
