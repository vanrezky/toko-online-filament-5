...
import { Heart, KeyRound, LogOut, MapPin, Package, Settings, User, CheckCircle2 } from "lucide-vue-next";
            ...
            <Card variant="elevated" class="overflow-hidden rounded-2xl border-0">
                <div class="from-primary/10 via-primary/5 bg-gradient-to-r to-transparent p-6">
                    <div class="flex flex-col items-center text-center">
                        <div class="relative">
                            <div class="bg-secondary flex h-20 w-20 items-center justify-center overflow-hidden rounded-full border-4 border-white shadow-md">
                                <img v-if="customer?.image || customer?.profile_photo_url" :src="customer.image || customer.profile_photo_url" class="h-full w-full object-cover" />
                                <User v-else class="text-muted-foreground h-10 w-10" />
                            </div>
                            <div class="bg-primary text-primary-foreground absolute -right-1 -bottom-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-white shadow">
                ...
                <div class="space-y-4 p-4">
                    <nav v-for="group in navigationGroups" :key="group.label" class="space-y-1">
                        <p class="px-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ group.label }}</p>
                        <Link v-for="item in group.items" :key="item.id" :href="item.href" class="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all" :class="isActive(item) ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'">
                            <component :is="item.icon" class="h-5 w-5" />
                            <span>{{ item.name }}</span>
                        </Link>
                    </nav>
                    <div class="border-t border-border pt-4">
                        <Link :href="route('frontend.logout')" method="post" as="button" class="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-red-500 transition-all hover:bg-red-50">
        ...
        <main class="min-w-0 space-y-6">
            <nav class="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 lg:hidden" :aria-label="t('labels.account.heading')">
                <template v-for="group in navigationGroups" :key="group.label">
                    <Link v-for="item in group.items" :key="item.id" :href="item.href" class="inline-flex shrink-0 items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold" :class="isActive(item) ? 'bg-primary text-primary-foreground' : 'bg-secondary text-foreground'">
