...
const createFilterState = (source = {}) => ({
    ...
    sort: Array.isArray(source) ? "newest" : source.sort ?? "newest",
...
const activeFilterChips = computed(() => {
    ...
    if (state.price_min || state.price_max) {
        chips.push({
            key: "price",
            value: "price",
            label: formatPrice(state.price_min || 0) + " - " + (state.price_max ? formatPrice(state.price_max) : "∞"),
        });
    }
    if (state.rating_min)
        chips.push({ key: "rating_min", value: state.rating_min, label: t("labels.filters.rating_chip", { rating: state.rating_min }) });
...
const pageLinks = computed(() =>
    (props.products?.meta?.links || []).filter((link) => /^\d+$/.test(String(link.label).replace(/&hellip;|<[^>]+>/g, ""))),
);
...
<template>
    <TemplateWrapper :shell="false" :title="t('meta.products.title')">
        <PageShell container :title="t('labels.products.default_title')" :description="t('labels.products.description')" class="pb-12 md:pb-16">
            <div class="mb-5 flex flex-col gap-4">
                <div v-if="hasActiveFilters" class="flex flex-wrap items-center gap-2 rounded-2xl bg-white p-3 shadow-sm">
                    <span class="text-foreground text-xs font-semibold">{{ t("labels.filters.active") }}:</span>
                    <span
                    ...
                    >
                        ...
                        <button
                            ...
                            :aria-label="t('labels.filters.remove', { label: chip.label })"
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-[280px_1fr]">
                <Card as="aside" class="hidden rounded-2xl border p-5 shadow-sm lg:sticky lg:top-20 lg:block lg:self-start">
                    <div class="mb-5 flex items-center justify-between">
                        <h2 class="text-foreground text-lg font-bold">{{ t("labels.products.filter_and_sort") }}</h2>
                ...
                <section aria-live="polite" class="min-w-0">
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div class="flex w-full flex-wrap items-center gap-2 sm:w-auto">
                            ...
                            <FormSelect
                                id="product-sort"
                                :model-value="draftFilters.sort || 'newest'"
                                class="w-40 sm:w-44"
                    ...
                    <div v-else class="border-border relative overflow-hidden rounded-3xl border border-dashed bg-white py-20 text-center">
                        <div class="relative z-10">
                            ...
                            <Button
                                type="button"
                                class="bg-primary text-primary-foreground rounded-full px-6 py-3 text-sm font-semibold"
                                @click="resetFilters"
                            >
                                {{ t("labels.actions.reset_filters") }}
                            </Button>
                    ...
                    <div
                        v-if="products.data?.length && (pageLinks.length > 1 || previousPage || nextPage)"
                        class="mt-10 flex flex-wrap items-center justify-center gap-2"
                        :aria-label="t('labels.products.pagination')"
                    >
                        <button
                            type="button"
        ...
        <div
            ...
            role="dialog"
            aria-modal="true"
            :aria-label="t('labels.products.filter_and_sort')"
        >
            <button
                type="button"
                class="bg-foreground/40 absolute inset-0"
                :aria-label="t('labels.actions.close')"
                @click="isFilterOpen = false"
            ></button>
            <div class="bg-background absolute inset-x-0 bottom-0 max-h-[92vh] overflow-y-auto rounded-t-3xl px-5 pt-4 pb-5 shadow-2xl">
                <div class="bg-border mx-auto mb-4 h-1.5 w-12 rounded-full"></div>
                <div class="border-border mb-5 flex items-start justify-between border-b pb-4">
                    <div>
                        <h2 class="text-foreground text-xl font-bold">{{ t("labels.products.filter_and_sort") }}</h2>
                        <p class="text-muted-foreground mt-1 text-sm">{{ t("labels.products.filter_description") }}</p>
                    </div>
                    <button
                        type="button"
                        class="text-foreground rounded-full p-2"
                        :aria-label="t('labels.actions.close')"
                        @click="isFilterOpen = false"
                    >
                ...
                <div class="border-border mt-6 grid grid-cols-2 gap-3 border-t pt-4">
