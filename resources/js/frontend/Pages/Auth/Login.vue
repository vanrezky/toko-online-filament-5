...
<template>
    <TemplateWrapper :title="t('meta.login.title')">
        <div class="flex min-h-[70vh] items-center justify-center bg-secondary/30 px-4 py-12 sm:px-6 lg:px-8">
            <div class="w-full max-w-md space-y-8">
                <div class="rounded-2xl bg-white p-8 shadow-sm">
                    <div class="mb-8 space-y-2 text-center">
                        <h2 class="text-2xl font-bold text-foreground md:text-3xl">{{ t('labels.auth.welcome') }}</h2>
                        <p class="text-sm text-muted-foreground">{{ t('labels.auth.login_subtitle') }}</p>
                    </div>
                    ...
                    <form class="space-y-5" @submit.prevent="submit">
                        ...
                        <div class="flex items-center">
                            ...
                            />
                            <label for="remember" class="ml-3 text-sm text-muted-foreground"> {{ t('labels.auth.remember_me') }} </label>
                        </div>
